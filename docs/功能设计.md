...
- 前缀 `commandPrefix`（默认 `/`），支持：`/help`、`/new`（切换到新会话，旧会话保留，可切回）、`/list`（列出会话代次）、`/switch <n>`（切回指定会话，上下文保留）、`/status`（登录态/队列/模型）、`/whoami`、`/cancel`（取消当前 turn）。
- `/new` 语义为**切换**而非丢弃：同一微信会话内普通消息永远复用当前会话；`/new` 开启新一代次（sessionId 追加代次），旧代次连同上下文完整保留，`/list` + `/switch <n>` 随时切回。代次记录持久化于 `$DSH_HOME/weixinbot/session-map.json`。
